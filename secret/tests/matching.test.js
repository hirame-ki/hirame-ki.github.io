...
/* ============================================================
   8. 부서별 행사 취합 파일
   한 줄이 학생이 아니라 "행사 하나" 인 서식. 참가자를 한 칸에 몰아 적고,
   이름 열을 "명단 · 참가자 · 참가 학생" 으로 적어 보낸다.
   전교생 대상 행사는 이름 칸이 아예 비어 있다.
   ============================================================ */
t.group("8. 부서별 행사 취합 파일");

/* 실제 파일에서 본 명단 한 칸 — 20명이 쉼표로 이어져 있었다 */
const PARTY = ["강나리","김가영","김서하","김연아","김예원","남혜린","박아림","배서현","서하린","심나영",
               "주다빈","최미주","최정민","김승준","박민성","박준성","정세훈","최현준","조재경","홍석현"];
const PARTY_CELL = PARTY.join(", ");
/* 우리 반은 그중 앞 8명뿐 — 나머지는 다른 반 학생이다 */
const EVENT_CLS = { id:"cE", name:"1학년 2반",
  students: roster(...PARTY.slice(0, 8).map((n, i) => ({ no: i + 1, name: n }))) };
const EV = "진로 체험 프로그램에 참여해 직업 현장을 살펴보고 소감을 정리해 발표하였다.";

t.eq("스무 명이 한 칸에 적혀 있어도 모두 쪼갠다",
  C.splitPeople(PARTY_CELL, EVENT_CLS.students).length, 20);

t.eq("한 반을 통째로 적은 서른 명짜리 명단도 쪼갠다",
  C.splitPeople(Array.from({ length: 30 }, (_, i) => "학생" + (i + 1)).join(", "),
                EVENT_CLS.students).length, 30);

/* 이름 열 머리글이 학교마다 다르다 */
["이름","명단","참가자","참가 학생","참여자 명단","학생 명단","신청자 명단"].forEach(head => {
  t.eq(`머리글이 "${head}" 여도 이름 열로 본다`,
    C.guessAcRoles([["연번", head, "일자", "내용"],
                    ["1", PARTY_CELL, "2025.09.01", EV],
                    ["2", PARTY_CELL, "2025.10.01", EV]], 0, EVENT_CLS.students)[1],
    "name");
});

t.eq('머리글이 없어도 명단 열을 값으로 찾아낸다',
  C.guessAcRoles([["1", PARTY_CELL, "2025.09.01", EV],
                  ["2", PARTY_CELL, "2025.10.01", EV]], -1, EVENT_CLS.students)[1],
  "name");

t.eq('"학생 활동 내용" 은 이름 낱말이 들어 있어도 내용 열로 본다',
  C.guessAcRoles([["연번","참가 학생","일자","학생 활동 내용"],
                  ["1", PARTY_CELL, "2025.09.01", EV]], 0, EVENT_CLS.students),
  ["ignore","name","ignore","text"]);

{ /* 담기까지 — 우리 반 학생만 골라 각자에게 담는다 */
  const rows = [
    ["1", PARTY_CELL, "2025.09.01", EV],
    ["2", "",         "2025.09.15", EV + " (전교생 대상이라 이름 칸이 비어 있다)"],
    ["3", PARTY_CELL, "2025.10.01", EV + " (다른 행사)"],
  ];
  const r = plan(["연번","참가 학생","일자","내용"], rows,
                 { cls: EVENT_CLS, myGrade: "1", myBan: "2" });
  t.eq("행사 두 건 × 우리 반 8명 = 16건을 담는다", r.fresh.length, 16);
  t.eq("여러 명이 적힌 줄 수를 알려 준다", r.multiRows, 2);
  t.eq("이름 칸이 빈 줄은 따로 세어 제외한다", r.noWho, 1);
  t.eq("다른 반 학생 12명은 못 찾음으로 알려 준다", Object.keys(r.unmatched).length, 12);
  t.eq("못 찾음은 이름 하나씩 보여 준다 (명단이 통째로 한 덩어리로 뜨지 않게)",
    r.unmatched["홍석현"], 2);
  t.eq("우리 반 학생은 행사마다 한 번씩 담긴다",
    r.fresh.filter(f => f.studentId === "s1").length, 2);
}

/* ============================================================
   9. 전교생 · 학년 대상 줄을 반 전체에 담기
   이름 칸이 비어 있고 "대상" 열에만 전교생 · 1학년 이라고 적힌 줄.
   교사가 옵션을 켰을 때만 담는다 — 한 줄이 학생 수만큼 늘어나기 때문이다.
   ============================================================ */
t.group("9. 반 전체에 담기 (targetScope)");

const TS = (v, g) => C.targetScope(v, g === undefined ? "1" : g);

t.eq('"전교생" 은 우리 반 전체',       TS("전교생"), "all");
t.eq('"전학년" 도 우리 반 전체',       TS("전학년"), "all");
t.eq('"전체" 도 우리 반 전체',         TS("전체"), "all");
t.eq('"1학년" 은 우리 학년이면 전체',  TS("1학년"), "all");
t.eq('"1학년 전체" 도 마찬가지',       TS("1학년 전체"), "all");
t.eq('"1.2.3학년" 처럼 여러 학년도',   TS("1.2.3학년"), "all");
t.eq('"1~3학년" 도 읽는다',            TS("1~3학년"), "all");
t.eq('"2학년" 은 우리 학년이 아니다',  TS("2학년"), "other");
t.eq('"신청자" 는 누구인지 알 수 없다', TS("신청자"), "");
t.eq('"명품반 29명" 도 알 수 없다',     TS("명품반 29명"), "");
t.eq('"IT경영21" 도 알 수 없다',        TS("IT경영21"), "");
t.eq("빈 칸은 알 수 없다",              TS(""), "");
t.eq("우리 학년을 모르면 학년 표기를 쓰지 않는다", TS("1학년", ""), "");

t.eq('"대상" 머리글을 대상 열로 본다',
  C.guessAcRoles([["연번","참가 학생","대상","내용"],
                  ["1","김민수","1학년",T1]], 0, CLS.students)[2], "target");

t.eq('"대상자 명단" 은 대상 열이 아니라 이름 열이다',
  C.guessAcRoles([["연번","대상자 명단","일자","내용"],
                  ["1","김민수, 이서연","2025.09.01",T1],
                  ["2","김민수, 이서연","2025.10.01",T2]], 0, CLS.students)[1], "name");

{
  const HEAD = ["연번","참가 학생","대상","내용"];
  const rows = [
    ["1","김민수","1학년",            T1],
    ["2","",      "전교생",           T2],
    ["3","",      "1학년",            T2 + " (우리 학년)"],
  ...
  const off = plan(HEAD, rows);
  t.eq("옵션이 꺼져 있으면 예전처럼 이름 있는 줄만 담는다", off.fresh.length, 1);
  t.eq("켜면 담을 수 있는 줄이 몇 줄인지 알려 준다", off.wholeOffer, 2);
  t.eq("이름 없는 줄은 모두 제외로 센다", off.noWho, 4);

  const on = plan(HEAD, rows, { classWide: true });
  t.eq("켜면 전교생 · 우리 학년 줄을 반 전체에 담는다",
    [on.wholeRows, on.fresh.length], [2, 1 + 2 * CLS.students.length]);
  t.eq("다른 학년 · 신청자 줄은 켜도 담지 않는다", on.noWho, 2);
  t.eq("켜고 나면 더 권하지 않는다", on.wholeOffer, 0);
  t.eq("반 학생 모두에게 같은 문구가 간다",
    on.fresh.filter(f => f.studentId === "s1").length, 1 + 2);
}

{ /* 대상 열이 없으면 켜도 담지 않는다 — 근거가 없다 */
  const r = plan(["연번","이름","내용"], [["1","",T1]], { classWide: true });
  t.eq("대상 열이 없으면 켜도 담지 않는다", [r.fresh.length, r.noWho, r.wholeOffer], [0, 1, 0]);
}

{ /* 이름이 적힌 줄은 대상 열이 있어도 이름을 따른다 */
  const r = plan(["연번","이름","대상","내용"], [["1","김민수","전교생",T1]], { classWide: true });
  t.eq("이름이 있으면 대상 열보다 이름이 먼저다", [r.names, r.wholeRows], [["김민수"], 0]);
}

/* ============================================================
   10. 전교생 파일 통째로
   ============================================================ */
t.group("10. 전교생 파일 통째로");
