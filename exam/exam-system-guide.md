...
- [x] (2026-09) **호실 입력 창구 일원화** — 호실을 정하는 곳을 **선택과목 응시자 명단 엑셀의 "호실" 열 한 곳으로 통일**. 고사 시간표 `[+ 과목]` 팝업의 "별실 호실" 입력칸을 제거하고(`exam_timetable.subjects`에 `room`을 더 이상 쓰지 않음), 일괄 배치·개인 시간표 모두 `subject_enrollment.room`만 참조하도록 변경. 호실이 비면 "호실 미지정"으로 한 묶음 처리하고 일괄 배치 결과에 경고 표시. 시간표 칸의 선택과목 칩에는 호실 대신 **응시자 명단 인원수**(없으면 "명단없음")를 표시해 과목명 오타를 바로 확인 가능. **SQL 불필요**(기존 시간표에 저장돼 있던 room 값은 무시되고, 그 시간표를 다시 저장하면 사라짐)
- [x] (2026-09) **첫 화면을 "결시 입력"으로 변경** — 로그인 직후 열리던 설정(고사 담당자용) 대신 담당 교사가 매번 쓰는 결시 입력 탭이 기본으로 열리도록 초기 활성 탭/페이지를 바꾸고, 로그인 성공 후 `showPage('input')`을 호출해 날짜·학급 선택까지 채워진 상태로 표시. **SQL 불필요**
- [x] (2026-09) **전체 학년 일괄 배치 + 제작자 칩 접힘** — ①별실·각자교실 일괄 배치 양쪽에 "전체 학년 한 번에 생성·저장" 버튼 추가(`generateBatchSeatCharts(allGrades)`로 통합, 학년별 명단·시간표를 순회하고 중복 갱신 키에 학년 포함, 건너뜀 메시지에도 학년 표기) ②우하단 제작자 칩을 평소엔 **인스타 아이콘만** 보이는 원형으로 두고 마우스를 올리면 0.9초 이징으로 스르륵 펼쳐지도록 변경(키보드 포커스·터치 시에도 펼쳐짐, `prefers-reduced-motion`이면 애니메이션 생략). **SQL 불필요**
- [x] (2026-09) **각자교실 일괄 배치 추가** — 별실처럼 각자교실도 버튼 하나로 **학년 전체 학급 × 모든 날짜·교시**의 배치표를 생성·저장. 교실 잔류 판정은 위 자동 계산과 같은 로직을 공유(`ownPlacement`)하고, 전원 별실 이동/자습 교시는 생성하지 않음. 같은 학급·날짜·교시 저장분은 갱신. **SQL 불필요**(코드 수정만)
- [x] (2026-09) **각자교실 응시자 자동 계산** — 각자교실 3단계에서 과목명 입력·응시자 클릭 선택을 없애고, 날짜·학급·교시만 고르면 시간표(공통과목)와 응시자 명단(선택과목+호실)으로 **이 교실에 남는 학생**을 자동 판정하도록 변경. 명단 호실이 "본인 교실"/자기 반 표기면 교실에 남고 그 외에는 별실 이동(미응시)으로 처리 → **한 반에서 일부만 별실로 가는 경우**를 입력 없이 반영. 과목명도 자동 입력되며, 손으로 고칠 수 있는 "직접 조정" 접힘 블록을 남김. **SQL 불필요**(코드 수정만)
- [x] (2026-09) **별실 자리배치 3단계 단순화** — 일괄 배치가 사실상 모든 경우를 처리하므로, 아래쪽의 과목 하나씩 직접 배치하는 영역과 **별실 응시자 엑셀 업로드 기능을 제거**하고, 과목명·호실·학생 클릭 선택만 남긴 **"예외 상황" 접힘 블록**으로 축소(시간표에 없는 과목 등 예외용). 저장된 배치표를 불러오면 이 블록이 자동으로 펼쳐짐. **SQL 불필요**(코드 수정만)
- [x] (2026-09) **유휴 자동 로그아웃 5분 → 2분 + 편집 내용 수시 임시 저장** — ①자동 로그아웃 시간을 2분으로 단축(`IDLE_LIMIT_MIN` 상수 하나로 관리, 배너·안내 문구의 `.idle-min`이 자동으로 따라감) ②서버에 바로 안 올라가는 두 입력칸(**고사 시간표 격자**, **학생 목록 텍스트칸**)을 편집할 때마다 브라우저 localStorage에 임시 저장해, 자동 로그아웃·새로고침·창 닫힘에도 내용이 남고 같은 학교코드로 다시 접속하면 그대로 복구됨(시간표는 `ttDraft_<학교코드>`에 저장하고 서버와 다르면 편집기 상단에 경고 배너 + "서버 내용으로 되돌리기" 버튼 표시, 학생 목록은 `stuDraft_<학교코드>_<학급id>`에 학급별로 저장). 서버 저장에 성공하면 임시본은 삭제. 결시 입력은 원래부터 학생을 누르는 즉시 서버에 저장되므로 그대로 둠. **SQL 불필요**(코드 수정만)
- [x] (2026-09) **과목 구분(공통/선택) 오등록 방지** — 사용자 제보("각자교실에서 응시자 우선 배치가 안 되고 미응시가 표시 안 됨")의 원인이 선택과목을 시간표에 "공통"으로 등록한 것(문구가 "공통=자기 교실에서 응시 / 선택=별실에서 응시"여서 교실 이동이 없는 학교는 공통을 고르게 됨 + 기본값이 공통)이라, ①선택지 문구를 "공통=반 학생 전체가 응시 / 선택=응시자 명단에 있는 학생만 응시"로 바꾸고 "교실 이동 여부와 무관" 안내 추가 ②신규 과목은 구분 기본값 없이 직접 고르게 함 ③응시자 명단이 있는 과목을 공통으로 저장하면 확인창, 시간표 칩에 `⚠명단 N명` 표시 ④각자교실 자동 계산·일괄 배치 결과에도 같은 경고 표시. **SQL 불필요**
- [x] (2026-09) **저장된 배치표 일괄 인쇄** — "저장된 배치표 불러오기" 창에 학년·날짜교시 필터, 항목별 체크박스, "보이는 N장 모두 선택", "교사 시선 배치표 포함" 옵션, **선택한 N장 인쇄** 버튼 추가. 새 창에 날짜·교시 → 학급/호실 순으로 배치표를 한 장씩(교사 시선 포함 시 두 쪽씩) 이어 붙여 가로 A4로 인쇄. 결시는 인쇄 시점 기준으로 반영. 좌석이 모자란 배치표는 건너뛰고 상단에 표시. 배치 그리기 로직을 `buildSeatGrid`/`seatGridHtml`로 분리해 화면 생성과 공유. **SQL 불필요**
- [x] (2026-09) **[버그 수정] 일괄 배치가 예전 배치표를 지우지 않던 문제** — 사용자 제보("선택학생이 0명인 교실(예: 2-1반 인공지능수학)이 전체 학생 응시로 출력됨"). 원인: 각자교실·별실 일괄 배치가 **생성·갱신만 하고 삭제는 하지 않아**, 시간표·명단을 고친 뒤(예: 선택과목을 공통으로 잘못 등록 → 선택으로 수정) 이제 그 교실에 응시자가 없는 교시에 **예전에 저장된 배치표(전원 응시)가 그대로 남아** 교실별 인쇄에 나옴(자습 채우기도 저장본이 있는 칸은 건너뜀). 수정: ①각자교실 일괄 배치 — 시간표 칸 중 그 교실 응시자가 0명(자습·전원 별실 이동)인 칸에 남은 own 배치표를 삭제 ②별실 일괄 배치 — 시간표 그 칸의 과목으로만 이뤄졌는데 이번 계산에서 어떤 호실 묶음에도 해당하지 않는 separate 배치표를 삭제(시간표에 없는 과목으로 직접 만든 예외 배치표·좌석 부족으로 건너뛴 호실은 보존) ③두 경우 모두 기존 저장본 조회를 확인창 **앞으로** 옮겨 "예전 배치표 N장 삭제"를 미리 안내, 결과에 "예전 배치표 정리 N장" 표시 ④교실별 전체 인쇄 목록에 **현재 시간표·명단으로 계산한 응시자와 다른 각자교실 배치표** 수를 교실별·상단 경고로 표시(재일괄 배치 안내). 기존 학교는 **각자교실·별실 일괄 배치를 한 번 다시 눌러야** 예전 배치표가 정리됨. **SQL 불필요**
- [x] (2026-09) **자동 로그아웃 2분 → 1분 + 마지막 시험 뒤 교시는 "하교"(빈칸)** — ①`IDLE_LIMIT_MIN`을 1로(배너 문구 `.idle-min` 자동 반영) ②그날 그 학년의 **마지막 시험 교시**(`lastExamPeriod`: 과목이 1개 이상 들어간 가장 늦은 교시) 뒤의 교시는 자습이 아니라 하교로 처리: 학생별 개인 시간표에서 "자습" 대신 빈칸(모든 날짜에서 시험이 없는 교시 줄은 아예 제외), 교실별/선택 일괄 인쇄의 "자습 교시도 포함"도 그날 마지막 시험 교시까지만 채움, 시간표 편집 격자의 빈칸에 회색 "자습"/"하교" 표시. 시험 사이에 낀 빈 교시는 기존대로 자습. **SQL 불필요**
- [x] (2026-09) **교실별 전체 인쇄 (일반 교실 + 별실 통합)** — 자리배치 3단계 카드 우측 상단(별실·각자교실 양쪽)에 **교실별 전체 인쇄** 버튼 추가. 학교 전체 저장 배치표(각자교실+별실)를 **실제 교실 단위**로 묶어, 교실마다 첫날 1교시 → 마지막 날 마지막 교시 순으로 이어 인쇄 → 뽑은 뒤 교실별로 그대로 배부. 순서는 일반 교실(1학년 1반 → 끝 학년 끝반) 다음 별실(이름순). 묶는 규칙: 각자교실 배치표 = 그 반 교실, 별실 배치표 = 호실명(“2-1반”/“2-1”/“2-1교실”/“2학년 1반”, 같은 학년 “1반”은 그 반 교실로 합침, 그 외는 호실명별 별실). 같은 반 교실·같은 교시에 각자교실 배치표와 별실 일괄 배치의 그 반 학생용 배치표(같은 학년)가 둘 다 있으면 같은 학생들이므로 각자교실 것만 인쇄. "일반 교실의 자습 교시도 포함"(기본 켬)이면 배치표가 없는 교시를 자습(전원 미응시)·자동 계산 배치표로 채우고, 저장본이 하나도 없는 반은 2단계 각자교실 좌석 설정으로 그림. 학년·날짜 필터, 교실별 체크, 교사 시선 포함 옵션, 예상 쪽수 표시. 교실별 인쇄에서는 쪽 제목을 **교실 이름**으로 크게, 아래 줄에 날짜·교시·과목. 기존 "저장된 배치표 불러오기"의 선택 인쇄는 그대로 유지(인쇄 창 코드 `writeSeatPrintWindow`로 공유). **SQL 불필요**
- [x] (2026-09) **한 교실 여러 과목 동시 시험 시 과목별로 모아 배치** — 사용자 제보("2과목이 섞여서 반번호 순으로 배치됨")에 따라, 한 배치표 안에 과목이 2개 이상이면 **A과목 응시자를 반·번호순으로 먼저 채우고, 끝난 자리부터 B과목을 반·번호순**으로 채우도록 변경(창가/복도 시작 방향은 기존과 동일). 과목 순서는 **고사 시간표 그 칸에 과목이 입력된 순서**. 각자교실은 과목별 응시자(결시 포함, 자리 유지) 뒤에 미응시 학생이 번호순으로 이어짐. 과목이 1개면 기존과 같은 번호순. 화면 생성·불러오기·PDF·일괄 인쇄 모두 같은 규칙(`annotateSeatSubjects`에서 정렬). 저장 데이터는 학생 목록만 가지므로 **기존 저장본도 다시 불러오거나 인쇄하면 새 순서로 나옴**. **SQL 불필요**
- [x] (2026-09) **일괄 인쇄 순서 선택 + 자습 교시 포함** — 일괄 인쇄 옵션에 "인쇄 순서" 추가: **학급별(기본)** = 1학년 1반부터 끝 학년 끝반까지, 한 반의 첫날 1교시 → 마지막 날 마지막 교시를 모두 출력한 뒤 다음 반(별실은 호실별) / **교시별** = 날짜·교시마다 1반 → 끝반. 각자교실에는 "선택한 반의 자습 교시도 포함"(기본 켬) 옵션을 둬, 체크한 반마다 그 학년 시간표의 모든 날짜 × 1~마지막 교시(교시 수 = 시간표 설정의 교시 수와 실제 시간표 중 큰 값) 중 **저장된 배치표가 없는 칸**을 채워 출력 — 자습 칸은 학급 전원 미응시 배치표("자습"), 시험이 있는데 저장 안 된 칸은 시간표 기준 자동 계산 배치표. 좌석 설정은 그 반의 저장된 배치표를 따르고, 체크만 해제한 저장본이 있는 칸은 자습으로 채우지 않음. 날짜·교시 필터를 걸면 그 교시만 채움. **SQL 불필요**
- [x] (2026-09) **배치표 상단에 고사 날짜·교시 표시 + 한 교실 여러 과목 동시 시험 대응** — ①배치표 제목 아래 줄에 "호실(학급) · 09월 14일 1교시" 표시(화면·인쇄·PDF·일괄 인쇄 공통) ②한 배치표 안에 과목이 2개 이상이면 좌석마다 이름 아래에 학생별 과목명 표시(시간표+응시자 명단 기준 계산, 과목이 1개면 표시 안 함) ③별실 일괄 배치가 기존 "과목×호실"별이 아니라 **"날짜·교시×호실"별로 한 장**을 만들도록 변경(과목명은 "물리학Ⅰ · 화학Ⅰ"처럼 합쳐 저장). 같은 호실·교시에 과목별로 따로 저장돼 있던 기존 배치표는 다시 일괄 배치하면 한 장으로 갱신되고 나머지는 삭제됨. **SQL 불필요**
...
CREATE TABLE IF NOT EXISTS subject_enrollment (
  ...
  student_id INT REFERENCES students(id) ON DELETE CASCADE,
  room TEXT,  -- 학생별 실제 호실. 호실을 정하는 유일한 곳(2026-09~). 비어있으면 "호실 미지정"으로 묶임. 한 과목이 여러 호실로 나뉠 때도 이 값으로 분할
...
> **이 시스템은 여러 학교가 하나의 Supabase 무료 프로젝트를 공유합니다.** 동시 접속(Realtime 연결) 한도가 200명이라, 사용 후 반드시 로그아웃하도록 안내되어 있고 1분간 조작이 없으면 자동 로그아웃됩니다 (대시보드 상단 배너 + 결시 입력 탭 안내 문구로 고지). 유휴 시간은 `index.html`의 `IDLE_LIMIT_MIN` 하나만 고치면 되고, 화면 안내 문구의 `.idle-min` 값도 자동으로 따라 바뀝니다.
...
1. **고사 시간표 입력** — 상단에서 교시 수와 교시별 시간(예비령/준비령/시작/종료)을 한 번만 설정하면 모든 날짜에 공통 적용됨. 시간은 **08:00~17:00 5분 단위** 드롭다운으로 선택(기존 직접입력 값이 이 범위·단위를 벗어나 있으면 그 값도 선택지에 보존됨). 그 아래 **학년(열) × 날짜·교시(행) 격자**의 각 칸에 `+ 과목`으로 과목을 추가(한 칸에 여러 과목 가능), 과목마다 **공통**(반 전체가 자기 교실 응시) / **선택**(별실 응시)을 지정.
   ...
   - 시간표에는 **호실을 입력하지 않음** — 선택과목이 어느 방에서 치러지는지는 아래 "선택과목 응시자 명단" 엑셀의 "호실" 열에서만 정함(입력 창구 일원화). 시간표 칸의 선택과목 칩에는 그 과목의 **응시자 명단 인원수**가 표시되고, 명단에 없으면 "명단없음"으로 떠서 과목명 오타를 바로 확인할 수 있음.
   - 비운 칸은 자습(단, **그날 그 학년의 마지막 시험 뒤 교시는 하교** → 개인 시간표·배치표 인쇄에서 빈칸). "고사 시간표 저장" → `exam_timetable`. (교시 시간은 localStorage에도 캐시되어 저장 전에도 유지됨. 저장은 학교 전체 시간표를 삭제 후 재삽입하는 방식이며, 삽입 실패 시 저장 전 상태로 자동 복구됨)
2. **선택과목 응시자 명단** — 학년 선택 후 양식(과목명/반/번호/이름/호실) 다운로드 → 선택과목 응시자만 채워 드래그앤드롭 업로드. 반+번호로 등록 학생과 매칭해 `subject_enrollment`에 저장(그 학년 명단 전체 교체). 공통과목은 반 전체라 입력 불필요. **호실 열이 호실을 정하는 유일한 곳** — 학생마다 실제로 가는 방을 적으면(예: 일부는 "3-1교실"에 남고 일부는 "시청각실"로 이동) 그대로 저장되고, 비워두면 "호실 미지정"으로 묶여 배치표에 방 이름이 표시되지 않음. 매칭 실패/이름 불일치는 경고로 표시, 반+번호 일치 시 이름 달라도 저장. 아래 요약 카드에 학년별 과목·인원과 함께 **호실별 인원 분포**(여러 호실이거나 미지정 학생이 있을 때만 표시, 미지정은 주황 강조)가 나타나고, "이 학년 명단 삭제" 버튼 제공. (호실 양식 칸은 텍스트 서식 고정 — "2-1"처럼 입력해도 엑셀이 날짜로 바꾸지 않음. 업로드 코드에도 날짜로 잘못 인식된 값을 복원하는 방어 로직 있음)
3. **학생별 개인 시간표** — 학년/학급(전체 가능) 선택 후 "개인 시간표 인쇄" → 시간표+응시자명단을 학생 기준으로 합쳐 새 창에 인쇄. **세로축=교시, 가로축=날짜** 그리드 표로 학생마다 표시(칸마다 과목명/장소, 신청 안 한 교시=자습). 공통=본인 교실(적용 학급 지정 시 해당 반 학생에게만 표시), 선택=**그 학생에게 지정된 호실**(명단에 호실이 비어 있으면 "별실(호실 미지정)"으로 표시). **A4 한 장에 2열×5행(10명)씩 절취선(점선 테두리)으로 구분해 배치**되고, 학생마다 카드 크기가 명함 정도(~95×55mm)로 작아 잘라서 개인별로 배부하기 좋음(11번째 학생부터 다음 장). 팝업 자동 인쇄.
...
2. 3단계 상단 **시간표 기반 일괄 배치(권장)**: 버튼 두 개 — **이 학년만 생성·저장** / **전체 학년 한 번에 생성·저장**(학급이 등록된 모든 학년을 순회, 저장 매칭 키에 학년 포함). 설정 탭의 고사 시간표에 입력된 **선택과목 전체** + 선택과목 응시자 명단을 읽어, **과목×호실 조합별로** 배치표를 **한 번에 생성·저장**(2단계 좌석 설정을 공통 그리드로 적용). 호실은 **응시자 명단의 "호실" 열만** 보고 나눔 — 같은 과목이라도 호실이 다르면 방마다 별개 배치표가 생성되고, 호실을 비워둔 학생들은 "호실 미지정" 한 장으로 묶이며 결과 메시지에 경고로 표시됨. 확인창에 "과목 N개(호실 기준 M개 배치표)"로 실제 생성될 개수를 미리 안내. 같은 과목·호실·날짜·교시로 이미 저장된 항목은 갱신, 응시자 명단 없음/좌석 부족(호실 단위로 판단)은 건너뜀(경고 표시). 생성 후 '저장된 배치표 불러오기'에서 개별 확인·인쇄·조정.
3. 3단계 아래 **"예외 상황 — 과목·응시자를 직접 골라 한 장만 만들기"**(기본 접힘): 시간표에 없는 과목이거나 이번만 명단과 다른 학생들로 한 장을 만들어야 할 때만 사용. 과목명/별실 호실을 직접 입력하고, 학년 전체 학생이 반별로 그룹핑된 목록에서 응시자를 클릭해 선택. (2026-09 이전에 있던 **별실 응시자 엑셀 업로드는 제거** — 응시자 등록은 설정 탭의 선택과목 응시자 명단으로 일원화)
...
5. 인쇄 버튼 옆 **현재 자리배치 저장** 버튼 → 날짜/교시/학년/과목명/호실/좌석 설정/선택 학생을 `seat_charts` 테이블에 저장. "3단계" 카드 우측 상단 **저장된 배치표 불러오기**로 목록을 열어 클릭하면 해당 설정이 그대로 복원되고 배치표가 즉시 재생성됨(이때 위 "예외 상황" 접힘 블록이 자동으로 펼쳐져 과목명·응시자를 바로 수정할 수 있음). 불러온 뒤 내용을 수정하고 다시 저장하면 새 항목이 아니라 **같은 저장 항목이 갱신**됨(버튼 라벨이 "다시 저장"으로 바뀜). 학년을 바꾸거나 다른 시험 방식으로 전환하면 새 저장으로 취급됨. 목록의 각 항목 옆 **삭제** 버튼으로 더 이상 필요 없는 저장 항목을 제거 가능(확인창 후 즉시 삭제, 되돌리기 불가). 현재 불러와 편집 중인 항목을 삭제하면 "저장" 버튼은 갱신이 아닌 새 저장으로 전환됨

**각자교실 시험** (자기 반 교실에서 그대로 응시)
1. 2단계: **날짜 / 학급 / 교시** 순으로 선택 / 교실 행·열 수 / 앞번호 시작 방향 / 좌석 편집(별실과 동일하게 칠판·창가·복도 표시 포함)
2. 3단계 상단 **일괄 배치(권장)**: 버튼 두 개 — **이 학년만 생성·저장** / **전체 학년 한 번에 생성·저장**(학급이 등록된 모든 학년의 모든 학급, 학년마다 그 학년 시간표·응시자 명단을 각각 적용). 2단계에서 고른 학급의 **학년 전체 학급 × 시간표의 모든 날짜·교시**를 돌며, 그 교실에서 시험이 있는 칸만 골라 배치표를 **한 번에 생성·저장**(2단계 좌석 설정을 모든 학급에 공통 적용). **전원이 별실로 가거나 자습인 교시는 만들지 않고**, 학급 인원이 좌석보다 많으면 건너뜀(경고 표시). 과목명은 자동 계산된 값(반마다 공통과목이 다르면 그 반의 과목명)이 들어감. 같은 학급·날짜·교시로 이미 저장된 배치표는 갱신
3. 그 아래 **이 학급·교시 한 장만** — 시간표 기준 자동 계산: 2단계에서 날짜·학급·교시를 고르면, 그 학년 고사 시간표의 **공통과목**(적용 학급 지정 시 그 반만)과 **선택과목 응시자 명단**을 읽어 **이 교실에 남는 학생**을 자동으로 골라내고 과목명도 자동으로 채움. 판정 규칙은 학생 한 명 기준으로 ①그 교시 선택과목 응시자면 → 명단의 호실이 "본인 교실"(또는 "3-2", "3-2교실", "2반" 등 자기 반을 가리키는 표기)이면 **남음**, 다른 방이면 **별실 이동 = 미응시** ②선택과목이 없고 그 반에 적용되는 공통과목이 있으면 **남음** ③둘 다 없으면 **자습 = 미응시**. 상태줄에 "이 교실 응시 N명 / 별실 이동 M명(과목 → 호실별 인원) / 자습 K명"이 표시됨 — **한 반에서 일부는 교실에, 일부는 별실로 가는 경우가 그대로 계산됨**
   - 그 아래 **직접 조정**(접힘): 과목명 수정, 학생 클릭 선택/해제, **전체 선택·해제**, **시간표 기준으로 다시 계산** 버튼. 시간표가 없는 날짜·교시는 자동 계산이 안 되므로(경고 표시) 여기서 직접 지정
4. 배치표 생성 → **응시자(체크한 학생, 결시자 포함)가 앞쪽에 번호순으로 몰려 배치**되고, 미응시(선택 안 한 학생)는 그 뒤로 번호순 배치됨. 응시자 그룹 안에 있는 결시자는 자리를 당기지 않고 **자기 자리를 빨간 점선 + "결시"로 빈자리 처리**함 (시험 규정 반영). 좌석 칸에는 번호만 표시
5. 별실 시험과 동일하게 **현재 자리배치 저장 / 저장된 배치표 불러오기**(3단계 카드 우측 상단) 지원 — 날짜/교시/학급/좌석 설정/선택 학생을 저장(`seat_charts.mode='own'`, `class_id`로 학급 식별)하고, 목록에서 선택하면 그대로 복원 후 즉시 재생성됨(이때 "직접 조정" 접힘 블록이 자동으로 펼쳐지고, 자동 계산이 저장된 선택을 덮어쓰지 않음)
...
- 1분간 조작이 없으면 자동 로그아웃(실시간 연결도 함께 정리됨), 로그아웃 버튼도 실시간 연결을 정리하도록 수정됨. 로그아웃 직전에 편집 중이던 내용을 임시 저장(`flushDrafts`)함
...
| subject_enrollment | school_code | 선택과목 응시자(학생↔과목). subject_name이 exam_timetable·seat_charts와 잇는 키. `room`(학생별 실제 호실)이 호실을 정하는 유일한 값 — 한 과목을 여러 호실로 분할 가능, 비어있으면 "호실 미지정". 업로드 시 해당 학년 전체 교체 |
| input_completions | school_code | 결시 입력 "반영 완료" 기록(학급/날짜/교시 단위), 고사본부 제출 현황판의 근거 데이터. absences와 함께 Realtime 구독 |

- Realtime 채널: 로그인 시 1회 생성(`abs-{SCHOOL_CODE}`), 로그아웃 또는 1분 유휴 시 해제. 같은 채널에 Presence를 붙여 헤더의 접속자 수 표시에 사용, absences/input_completions 변경 시 고사본부 자동 새로고침
...
- **선택과목 호실이 여러 개로 나뉘는 경우** (2026-07 도입): 한 별실에 학년 전체가 못 들어가거나, 같은 과목 응시자 중 일부만 별실로 이동하고 일부는 본인 교실에 남는 등 "과목=호실 1개" 가정이 안 맞는 학교가 있어, 호실을 **학생 단위**로 지정하도록 변경. `subject_enrollment.room`에 학생별 실제 호실을 넣고 이 값만 사용함(개인 시간표 표시, 일괄 자리배치의 그룹 분할 기준 모두 동일). **2026-09 후속 정리**: 입력 창구가 두 곳(시간표의 과목 기본 호실 + 명단의 학생별 호실)이라 담당자가 헷갈린다는 피드백에 따라 시간표 쪽 호실 입력을 제거하고 명단 한 곳으로 일원화함 — `exam_timetable.subjects[].room`은 더 이상 읽지도 쓰지도 않음(구 데이터에 남아 있어도 무시). 시간표 기반 일괄 배치는 이제 "과목" 단위가 아니라 "과목×호실" 단위로 배치표를 생성 — 같은 과목이라도 호실이 다르면 별개의 `seat_charts` 행으로 저장됨(매칭 키에 `room_name` 포함)
