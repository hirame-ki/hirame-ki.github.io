...
> **이 시스템은 여러 학교가 하나의 Supabase 무료 프로젝트를 공유합니다.** 동시 접속(Realtime 연결) 한도가 200명이라, 사용 후 반드시 로그아웃하도록 안내되어 있고 2분간 조작이 없으면 자동 로그아웃됩니다 (대시보드 상단 배너 + 결시 입력 탭 안내 문구로 고지). 유휴 시간은 `index.html`의 `IDLE_LIMIT_MIN` 하나만 고치면 되고, 화면 안내 문구의 `.idle-min` 값도 자동으로 따라 바뀝니다.
...
- 2분간 조작이 없으면 자동 로그아웃(실시간 연결도 함께 정리됨), 로그아웃 버튼도 실시간 연결을 정리하도록 수정됨. 로그아웃 직전에 편집 중이던 내용을 임시 저장(`flushDrafts`)함
...
- Realtime 채널: 로그인 시 1회 생성(`abs-{SCHOOL_CODE}`), 로그아웃 또는 2분 유휴 시 해제. 같은 채널에 Presence를 붙여 헤더의 접속자 수 표시에 사용, absences/input_completions 변경 시 고사본부 자동 새로고침
