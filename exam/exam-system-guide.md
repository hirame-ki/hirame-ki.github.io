...
1. 2단계: 날짜 / 교시 / **학년** / 교실 행·열 수 / 앞번호 시작 방향(창가·복도) 입력
2. 3단계: 과목명 / 별실 호실 입력 → 선택한 **학년 전체 학생**(학년-반-번호-이름)이 나열됨 → 응시자를 클릭해서 선택
3. 배치표 생성 → 선택된 학생이 **반-번호 순**으로 자동 배치됨

**각자교실 시험** (자기 반 교실에서 그대로 응시)
1. 2단계: 날짜 / 교시 / **학급** / 교실 행·열 수 / 앞번호 시작 방향 입력
2. 3단계: 과목명 입력 → 선택한 **학급 학생 명단**이 나열됨 → 응시자를 클릭해서 선택 (결시자는 자동 제외되어 배치에서 빠짐, 선택 안 된 나머지는 미응시로 표시)
3. 배치표 생성

**공통**
- 좌석 수(행×열)보다 배치할 학생이 많으면 오류 안내 후 행/열을 늘려야 함
- 인쇄 버튼 → 자리배치표(과목명/교실명 상단 + 좌석배치 + 요약)만 인쇄됨, 설정 화면은 인쇄 안 됨
