...
### 경주 트랙 게이트 퀴즈 버그 수정 (`ssambus_map_race.html`)
* **문제**: 교사가 AI로 미션을 생성해도 게이트에서 계속 내장 기본 문제("달리기 경주 출발 신호 색깔은?" 등)만 뜬다는 피드백
* **원인**: `showGateQuiz()`의 문제 풀 필터가 `m.type==='quiz'`(객관식)만 허용하도록 하드코딩돼 있어, O/X·주관식·토론으로 생성한 미션은 게이트에서 전혀 인식되지 못하고 매번 `RACE_DEMO_MISSIONS`로 대체됐음
* **수정**: 게이트 문제 풀에 `ox_quiz`, 이어서 `short_answer`까지 순차 지원 추가. `discussion`(토론/토의)만 정답 개념이 없어 구조적으로 미지원
* 교사 대시보드의 AI 문제 생성 모달 + 미션 관리 화면 상단에 "경주 트랙은 객관식·O/X·주관식만 게이트에서 출제됨" 안내 문구 추가

### 경주 트랙 주관식 정답 판정 (`ssambus_map_race.html`)
* 주관식 미션이 뽑히면 선택 버튼 대신 텍스트 입력창을 렌더링(Enter 제출 지원)
* 판정: 공백/대소문자/일부 문장부호만 무시하는 정규화(`__rqNormalize`) 후 완전 일치 비교(의미 유사도 판정 아님)
* 예시 답안(model_answer)을 비워두고 만든 미션은 채점 없이 아무 값이나 입력하면 통과

### 주관식 채점 로직을 전체 맵에 통일 (`ssambus_missions.js`)
* 기존에는 일반 맵(교실·도서관 등)의 주관식이 **채점을 전혀 하지 않고** 빈칸만 아니면 즉시 완료 처리됐고, 예시 답안은 제출 "후"에 참고용으로만 노출됐음
* 공용 정규화 함수 `__msNormalizeAnswer()` 추가(경주 트랙과 동일 기준)로 통일
* 예시 답안이 있는 미션은 학생이 **"정답 확인"** 버튼을 눌러야 채점되고, 맞아야 "완료" 버튼이 활성화됨(틀리면 재입력 가능)
* 예시 답안을 비워둔 미션은 기존과 동일하게 채점 없이 입력만 하면 완료

### AI 자동문제 생성 입력 모드 확장 (`ssambus_teacher_dashboard.html`)
PDF 업로드 단일 입력만 지원하던 "✨ AI 자동문제 생성" 모달에 입력 모드 탭 4종 추가(기존 PDF 흐름은 그대로 유지, 하위 호환):
* **📄 PDF 업로드**: 기존과 동일
* **📝 주제 입력**: 자료 없이 주제(+선택적 세부 설명)만 입력하면 AI가 자체 지식으로 문제 생성
* **🔗 웹페이지 링크**: `r.jina.ai` 무료 리더 API로 URL 본문 텍스트를 가져와 문제 생성(키 불필요, CORS 허용). 유튜브 URL은 자막이 아닌 페이지 정보만 가져와져 정확도가 낮을 수 있음(기술적 한계, UI에 명시)
* **🔄 유사 문제 생성**: 현재 맵의 기존 미션을 드롭다운으로 고르거나 직접 입력하면, 같은 주제·난이도의 새 문제를 다른 표현으로 생성
* 공용 Groq 호출 함수(`__pdfCallGroq`)에 `promptKind`(`material`/`similar`/`fill_options`) 매개변수만 추가해 프롬프트를 분기, 기존 PDF 호출부는 매개변수 생략 시 이전과 100% 동일하게 동작

### 보기·정답 자동생성 (`ssambus_teacher_dashboard.html`)
* 수동 "미션 추가" 모달의 퀴즈 질문란 옆에 "✨ AI로 보기·정답 채우기" 버튼 추가
* 질문만 입력하고 누르면 Groq가 4지선다 보기와 정답을 자동으로 채워줌(배치 생성이 아닌 단일 미션 보조 기능)
...
현재 구현 완료 목록: 맵 15종 · 애니 캐릭터 26종 · 맵 에디터(가구 배치·픽셀 아트·팔레트) · 미션 트리거 타일 · 채팅 모드(전체/근접/금지) · 욕설 필터 · 다양한 미션 타입(youtube/quiz/google_form/link/image_quiz/short_answer/discussion/ox_quiz) · AI 자동문제 생성(Groq API, PDF·주제입력·웹링크·유사문제 4모드 + 보기·정답 자동생성) · 주관식 채점 통일(전체 맵 공통, 예시 답안 정규화 후 정확 일치 판정) · 경주 트랙 게이트 퀴즈(객관식·O/X·주관식 지원) · 이전 수업 맵 불러오기 · 소감 입력 + 엑셀 반영 · 캐릭터 부드러운 이동 · 실시간 완료 순위 · 교사 참가자 미션 면제.
